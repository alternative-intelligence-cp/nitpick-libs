...
| s1 | `nitpick-regex` | **0.0.2 — the harness, part 1** | `s1-nregex-0.0.2-0152` | 2026-09-04 01:52 | `claude-opus-5` | **0.0.1 DONE and VERIFIED PASS** at `3679bf8` after three re-dispatches and four verifications. The false causal claim is corrected and *measured*, in a new §G kept separate from §F's reading of source. **`BUILD.md` B-7 (D-237) already required a rejection fixture's diagnostic-code set to EQUAL its expectations** — so the ecosystem already had the rule that makes the corrected hazard unreachable, and the false claim's only cost was pointing a harness author at the exit code alone. **0.0.2 is where the night's findings are spent:** it builds the runner that judges by exit code, so the alphabet, the one-byte limit and B-7 all land here. Carries RX-119 |
