...
|---|---|---|---|---|---|---|
| s2 | `nitpick-time` | 0.0.0 — the language probes | *(parked — no live agent)* | 2026-09-04 00:00 | `claude-opus-5` | **STOPPED on O-N4, O-N9, O-N10, O-N11 — all four in the compiler's 1.5.1b.** Maintenance dispatch **VERIFIED PASS** at `5b2e0c8`: RX-111/TM-108's bounds correction landed as *narrow-read-as-broad* rather than false, the `buffer` finding **enlarged** (`Bytes` is unchecked exactly as `Vec<T>` is, and every formatter goes through one), O-N10/O-N11 de-hedged at all nine current-fact sites, the 0.0.2 residue checklist line added, and the house view rule recorded as a belt stricter than the language. **Every debt that does not need the compiler is now cleared.** **Owed at the re-pin, and now three things:** probes 09 and 10; re-recording `missing_failsafe`'s two transcripts to `NITPICK-REACH-003`; and correcting **five** sites that state *DEF-3 adds no new diagnostic code* — `meta/DECISIONS.md:952`, `meta/OPEN_QUESTIONS.md:212`, `meta/specs/SAFETY.md:432`, `tests/probe/defect/view_escape/README.md:120`, and `meta/roadmap/0.4/README.md:102`, the last sitting in a **future cycle's** planning checklist. DEF-3's step 2 does add one, `NITPICK-BORROW-012`. These belong at the re-pin rather than now, because they are claims about DEF-3's landed behaviour and the landing is when a worker can check them against a real compiler instead of a worktree |
| s1 | `nitpick-regex` | 0.0.1 — the skeleton | `s1-nregex-0.0.1-verify-0055` — verifier | 2026-09-04 00:55 | `claude-opus-5` | **The FAIL is fixed and reported DONE; verifier RUNNING.** Four commits. **The one wrong exit code sat on an ecosystem-wide gap:** nothing documents what `npkc`'s exit codes MEAN, while `BUILD.md` B-6 orders every harness here to assert on those integers. The alphabet — 0 success, 1 REFUSED, 2 the driver judged nothing, 3 a trap — is now in `PLAYBOOK.md`. **The dangerous half: a `use` naming a missing path exits 2, not 1**, so a harness reading nonzero as "refused" scores a MISSING FILE as a passing rejection test, and every library imports by relative path until O-N2 closes. **CI ticked with evidence** (run `33835762747`). Remaining and deliberate: `probe` undeclared in the manifest (RX-119), marked half-done in both checklists and left to 0.0.2, which owns the runner and can move the paths and fix 0.0.0's citations in one commit |
