...
|---|---|---|---|---|---|---|
| s2 | `nitpick-time` | **0.0.5 DONE — VERIFIED PASS** at `0c85648` (one FAIL raised and **adjudicated in the worker's favour on the orchestrator's own measurement — the fault was the dispatch's**). Harness **GREEN, 40 units, ~43 s**. **`ahead 5` is correct.** Next: **0.0.6 (close)**, not dispatched | *(no live agent)* | 2026-09-05 23:02–23:5x | `claude-opus-5` worker, small-model verifier | **THE NUMBER THIS SUBCYCLE EXISTS FOR: 475 006 B for four tables and two pools, 489 310 B (477.8 KiB) with `POSIX_RULES` — against a ≈356 119 B estimate, LOW BY 37%.** Read off the object with `nm -S`, not computed, because **two of the three estimated row widths were wrong** (`ZoneTransition` 12 v **16**, `ZoneEntry` 16 v **28**). **TM-007 STANDS**; margin to the next band is **22 690 B = 1 418 transitions ≈ 4.4%**, now written into `ZONE_MODEL.md` as Z-7b rather than left for a tzdata release to discover. **AND IT FOUND A USE-AFTER-FREE THAT EXITS 0 — now O-N19, accepted upstream as a soundness hole and going to the author as a decision.** `TYPE-046` is not enforced inside a generic body, so a bare copy of an owning element at an unsubstituted `T` compiles, links and runs; the case that drops the first owner and reads the second **exits 170, the allocator's `0xAA` poison**. Reproduced by the orchestrator end to end. **THIS LIBRARY SHIPPED IT AT 0.0.4** — `vec_pop<T>` was the defect's own case verbatim, written, reviewed, **independently VERIFIED PASS** and committed, because `npkc` accepted it and every gate here is a leak gate. **A leak is found by a gate; this was found by a wrong answer.** Fixed at 0.0.5, and the compiler session ratified `move(s[…])` as the spelling the language means at every `T`. **TM-137 — an exemption list's both-directions diff checks the FILE EXISTS, not that its REASON HOLDS.** Two entries predicted *in their own text* that they would fail the day O-N17 landed; it landed, both files began running clean, **and the suite stayed GREEN with both stale**. Fourth instance of §6's shape, this one **inside the mechanism written to prevent it**. `check_exemptions_live` now re-derives each exemption's recorded verdict every run, and the verifier saw it go red. **TM-132 is NOT lifted, and that is correct** — the library's drop-loop spelling is refused `MOVE-001`, and the spelling that works would rest on the rule O-N19 shows the compiler cannot enforce (`compiler-defects-block-not-house-rules`). Cost of holding is nil: `ntime` instantiates no owning `T` by design |
