...
| s1 | `nitpick-regex` | 0.0.1 — the skeleton | `s1-nregex-0.0.1-0110` | 2026-09-04 01:10 | `claude-opus-5` | **Verifier FAIL on one CAUSAL claim, and the orchestrator had already propagated it into `PLAYBOOK.md`.** "A `use` naming a missing path exits 2" is **false** — a missing import exits **1** with `NITPICK-RESOLVE-005`; only a missing **root argument** reaches `pass 2i32`. Measured 3× by the verifier and 2× here. **The alphabet, the fifteen `exit 2` sites and `3` being real all HOLD** — the verifier counted them independently and matched site for site. **The corrected hazard is sharper:** a missing import gives exit 1, *the code a rejection fixture expects*, so a typo'd or moved fixture path passes for the wrong reason — hence a rejection test must assert the **diagnostic code**, not the exit code alone. Playbook corrected with a dated note; worker re-dispatched to correct the four sites it travelled to |
