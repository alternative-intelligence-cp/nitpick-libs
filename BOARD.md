...
|---|---|---|---|---|---|---|
| s2 | `nitpick-time` | **0.0.5 — the tzdb size spike — DISPATCHED 2026-09-05 23:02** against the NEW pin `aaffb87`. 0.0.4 **DONE — VERIFIED PASS** at `06f82c0` (`src/core/` exists, 35 public names, harness GREEN 40 units). **`ahead 4` is correct** | `s2-ntime-0.0.5-2302` | 2026-09-05 23:02 | `claude-opus-5` | **THIS IS THE CYCLE O-N4 USED TO BLOCK, AND IT IS NOW THE CHEAPEST IT WILL EVER BE.** The spike emits a real zone table and *measures* it, because TM-007 — the compiled-in tzdb, the decision every specification here rests on — currently costs an **estimate** (447 zones, 26 838 transitions, ≈348 KiB from TZif headers). **Estimates of emitted size are wrong in both directions routinely**, and finding at cycle 0.5 that the real answer is 4 MiB would reopen a decision the whole repository depends on. At `aaffb87` `probe04`'s 30 000-row table compiles in **1.18 s at 26 MB**, against 281 s and 30.9 GiB when O-N4 was open — so the measurement that justified this subcycle's existence now costs an afternoon rather than a machine. **INHERITED, AND NEWLY POSSIBLE THIS HOUR: TM-132's RESTRICTION IS LIFTABLE.** `vec_pop`, `vec_set`, `vec_clear`, `vec_truncate` and `vec_free` ship in `src/core/vec.npk` **restricted at an owning `T`**, on O-N17. **O-N17 is FIXED at this pin — verified here, all five reproduction cases now link and write objects.** So the restriction can be lifted and TM-132 amended; the worker verifies the *library's* spelling rather than assuming the compiler fix carries. **AND ONE TRAP THIS PIN INTRODUCES:** modules now hold **only the prelude functions they reference**, so any probe asserting a prelude symbol in IR needs a **use**. `nitpick-time` had none — but a generator emitting a large table is exactly the shape that might |
