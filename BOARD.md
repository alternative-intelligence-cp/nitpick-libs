...
|---|---|---|---|---|---|---|
| s2 | `nitpick-time` | **0.0.6 — THE CYCLE CLOSE — DISPATCHED 2026-09-06 with `AUDIT:`** naming [`meta/audits/nitpick-time-0.0-2026-09-06.md`](meta/audits/nitpick-time-0.0-2026-09-06.md). 0.0.5 DONE — VERIFIED PASS at `0c85648`; harness GREEN 40 units ~43 s | `s2-ntime-0.0.6-0030` | 2026-09-06 00:30 | `claude-opus-5` | **THE AUDIT FOUND FIVE IT WOULD NOT CLOSE OVER, AND THE ORCHESTRATOR REPRODUCED THE THREE THAT CHANGE WHAT THE CLOSE MUST DO.** **A2 IS THE ONE TO READ FIRST: a SECOND use-after-free on this library's own public surface.** `bytes_view`'s comment promises the view *"is valid exactly as long as the `Bytes` is"*, while `bytes_reserve` **drops the old buffer on growth** — a view held across a push reads **170**, the allocator's poison. Both functions are public. **It is invisible for the same structural reason as `vec_pop<T>` at 0.0.4: every gate this repository owns is a leak gate, and a use-after-free is found by a wrong answer.** **A1 — `check_no_owning_fields` cannot see a single-line struct, and BOTH of this repository's structs are one-liners**, so neither type's real fields has ever been examined; the self-check plants the violation *multi-line*, so the check is red on the fixture its author wrote and silent on the same fault in the form the repository actually uses. **The exact shape the audit was asked to hunt.** **A3 — CI will not go green:** `toolchain.py` matches the version on `llc --version`'s **first line only**, and the upstream tarball CI installs is built without `PACKAGE_VENDOR`, so its banner puts the number on line two — verified at LLVM's own source at tag `llvmorg-20.1.2`. **A4 — CI pins `0dfddac`** under a comment claiming it is this board's pin, which reads `aaffb87`. **A5 — `check_exemptions_live`, the mechanism 0.0.5 built to fix TM-137, is in neither the specification nor the self-check**, and `TESTING.md` V-14c's *"every check is commissioned"* is false. **W-22: the close worker triages EVERY finding — A through F — and defers only with a reason.** The audit also verified a great deal clean, including all 51 cited compiler decisions resolving at the pin, the tzdb arithmetic, and the calendar bounds recomputed independently |
