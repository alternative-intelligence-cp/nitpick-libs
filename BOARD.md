...
|---|---|---|---|---|---|---|
| s2 | `nitpick-time` | **0.0.4 — `src/core/` — RE-DISPATCHED 2026-09-05 21:03 with `TREE: dirty`, inheriting its predecessor's uncommitted work (§4).** The first worker died at ~20:50 when the session was killed mid-run. The session was killed at ~20:50 while the worker was live; **nothing was lost** — 6 tracked files modified, 7 new probe files, and a complete defect reproduction, all uncommitted. 0.0.3 DONE at `60e03bf`, 0.0.2 at `e101312`, 0.0.1 at `0c7e156`; **`ahead 3` still correct** | `s2-ntime-0.0.4-2103` (re-dispatch) | 2026-09-05 21:03, TREE dirty | `claude-opus-5` | **IT FOUND A COMPILER DEFECT AND STOPPED RATHER THAN ROUTING AROUND IT — now `O-N17`, raised upstream.** A generic function moving OUT of an indexed element at an owning `T` calls a `@npk.vacant.<n>` helper the emitter never defines: `npkc` **exit 0** writes the `.ll`, `llc` **exit 1** writes no object. **Reproduced by the orchestrator before the board moved.** Three controls put the fault at exactly *generic + owning + move-out*. **The actionable half is a count:** all four cases **define the same five** vacant helpers, the three controls **call three** of them, the failing one **calls four** and the fourth callee is not among them — so the call site synthesises a `dty` the definition walk never visits. **`vec_pop<T>` is HELD, not rewritten** (W-11). **Blocks one row of one API table, nothing else here** — `ntime` plans no `Vec<T>` at an owning `T`. **THE NUMBER WAS WRONG AND IS FIXED HERE:** the worker filed it as `O-N12`, which is `nitpick-regex`'s settled `>>>` question; the registry ran O-N1…O-N16, so **O-N17** is assigned, and **TEN citations across NINE files still say `O-N12` and must be corrected at the re-dispatch** — the orchestrator first published *seven*, from a sweep of two directories with its denominator unstated, and the full sweep (136 tracked + 7 untracked paths) found three more in `harness/run.py` and the two `probe12` files. The rule that catches this is on this board and in `PLAYBOOK.md`, written by the session that then broke it. **Three other findings are in the dirty tree and must not be lost:** **TM-128** — *the acceptance's own "under 768 KiB" figure is NOT reproducible*, the real bound being a 3 MiB address space, found because the dispatch said to re-measure rather than inherit it; **TM-127** — overwriting a `Vec<T>` element discards one, so `vec_set` is a fourth entry owing an element drop; **TM-130** — ten `VerificationKeyword` spellings are reserved and none was in any table here | **THREE COMMISSIONED CHECKS GO FROM A DENOMINATOR OF ZERO TO A REAL ONE THE MOMENT THIS LANDS** — `check_raw_index` (whose `.items[`/`.ptr[` owners are the very files this subcycle writes), `check_constants_named` (`limits.npk`) and `check_no_owning_fields`. **They are already red-tested**, so a violation in the first draft of `Vec<T>` is a red run rather than a review comment — which is the whole return on 0.0.3 having built the self-check before any code existed. **The leak acceptance already carries RX-126's correction and needs no repair here:** the `exit 0` gate is about **the `wild` BLOCK alone** — D-151 cannot see a managed body, so an unemptied `Vec<string>` passes it — and the element leak has **its own gate, a MEMORY CAP rather than an exit code** (`probe06b`/`probe06c`, one line apart, both exit 0; the leaking half peaks **125 184 KiB** and takes `HeapOom` **exit 92** under `ulimit -v 65536`, the corrected half finishes under **768 KiB**). **That 125 184 KiB was measured at the OLD pin `94874ce` and is independently corroborated as still current** — 0.0.3's full-run peak RSS at `0dfddac` is the same 125 184 KiB — **but the worker re-measures rather than inherits it.** **`check_constants_named`'s owner map assigns all four magic numbers to `cal`** because `SAFETY.md` S-16 says the *calendar* algorithms divide by them; if 0.2 gives `span` its own nanosecond arithmetic, **amend S-16 and the map in ONE commit** — the check's contract is that it fires when tree and document disagree, and the fix is whichever of the two is wrong | **THE VERIFIER RETURNED PASS AND TWO OF THE NINE PRESSED POINTS WERE NOT ACTUALLY TESTED — the orchestrator closed both before the board moved, and the pattern in WHICH two is the finding.** (a) **V-15's stopping** was reported as *"a self-check failure **would** block subsequent stages"* — an inference from reading the code, where the dispatch had asked for a planted failure. **Planted here: `run.py` exit 1, only `[1/9] self` in the output, stages 2–9 never ran**, and the tree restored byte-identical. (b) **The arm-bill oracle** was checked on six `src/` placeholders that all owe the floor of **4**, which cannot distinguish the claim from its negation; **the specimens that differ were re-run here** — `probe11c` (calc_lib, floor-only handler) gives **4× `NITPICK-REACH-002`** naming `DivByZero`, `DivOverflow` and two more, so **+4 → 8**; `probe11b` names `probe11_arms_lib.EProbeZone`, so **+1 → 5**. Both bills confirmed. **Everything else the verifier reported it had genuinely re-run.** **THE SHAPE: both gaps were checks needing SETUP — planting a fault, choosing a discriminating specimen — and in both the verifier substituted reading for running, correctly reasoning its way to the right answer.** It re-runs commands reliably and reasons about anything that needs construction, and **reasoning is what the worker already did**, so that half of the verification is not independent. **This is the second such gap today** (the other carried a `56` that its own arithmetic falsified). **Fix, applied from the next dispatch: give the verifier the LITERAL plant command, or verify setup-requiring claims at the orchestrator.** **TM-124 — the `accept` stage was DECLINED, not skipped**, and the strike is legitimate: the plan listed it while `BUILD.md` B-4b and `TESTING.md` §1 both say this library does not use it, and **the specifications are the authority (TM-002), so the plan was the wrong document**. Verified active: a manifest naming `accept` is **refused by name with its reason**, distinguished from *"not yet implemented"* | **THE SELF-CHECK IS THE LOAD-BEARING HALF — everything else this subcycle builds is a check, and `selfcheck.py` is the only thing that shows the checks can fail.** 0.0.2 stated plainly what it did NOT prove, and the list is 0.0.3's work: **not that the runner can fail** (three instruments were commissioned by hand; that is three checks, not a runner), **not purity** (the symbol scan cannot see a syscall — TM-118, RX-120 — so `check_purity` must be **source-level**), and **not that CI is green** (still never run). **0.0.2 left four specimens rather than descriptions:** `selfcheck.py`'s case 3 — a reported code no expectation names — **has a real specimen in this tree's history**, `probe02d` until 0.0.2 fixed it, so use it rather than inventing one; the **eighth case** (a deleted `failsafe`) is already guarded by `build_program`'s `require_failsafe`, kept deliberately though redundant under DEF-5; `check_layering` already has its walk in `build.reachable_sources`, which follows `use` with a seen-set because a `use` cycle is legal (D-086); and a `--quick` that skips `sweep` **must print its loud line through the same `Report` object**, or summary and transcript can disagree — the exact failure `--verdicts` exists to prevent | **P-16 WAS UNEXECUTABLE AND THE COMPILER WAS RIGHT ALL ALONG (TM-117).** "Compile the library once, link each program against it" is impossible: two `npkc` objects are a duplicate-symbol error (`ld.lld` exit 1, 121 lines, `npk.prelude.int8:ToString.to_string` twice), because every compile emits the whole reachable graph *including the prelude* — `import.npk`, which computes nothing, emits **845 282 B**, the canary number exactly. **But `BUILD_REFERENCE` §4.1 says so at the pin** — *"takes one program object and adds the runtime object; there is no parameter through which a third input could enter"* — **so this is OUR planning error, not a compiler defect**, and P-16 is the natural thing to write, so **the other four libraries' harness plans probably carry it. Relay TM-117 at each next claim.** **Three negative controls were rebuilt and re-run by the verifier, not accepted:** the symbol scan red on an introduced undefined symbol, the toolchain pin red on 20.1.3-vs-20.1.2, and `repro` red on an unsorted generator with a sorted control green through the same code path. **That repository's own open question on the `[[test]]` split — one entry cannot cover `tests/probe/` when seven of its twenty-six must not compile — is SETTLED as TM-119** (dispatch on the file's own header; 19 `expect-exit:` and 7 `expect-error:`, re-measured before deciding). **Its `O-`number is deliberately not written here: the workbench's `O-N*` and a library's `O-X*` share one namespace as far as `check_refs` is concerned** (`\b(O-[A-Z]\d+|Q-\d+)\b`), so naming a library's open question on this board makes the check demand a definition **this** repository's `meta/OPEN_QUESTIONS.md` must not contain. **It fired here, on this row, and gated the commit** — the same hazard this board already records for `RX-` numbers, now shown to apply to `O-` as well and to be enforced rather than merely stated. **`// env:` marker added (TM-120)** and the harness **constructs** the environment rather than inheriting it — verified by passing with a hostile `TZ=UTC` exported. Nine leak-comment probes reworded, **residual 0**, `probe04_big_fixed_table.npk` among them — the one probe nobody compiles, which drops out of any list built from what a session ran | **THE SUBCYCLE'S OWN VALUE IS THE SKELETON; ITS LASTING VALUE IS FOUR CORRECTIONS IT MADE TO WHAT IT WAS TOLD.** (a) **`case3` names SIX identities, `case1` four** — this board had generalised one program's floor to the set; re-measured by the orchestrator before the board moved, see the O-N11 row. (b) **`TZ=Europe/KIEV` exits 34, not 0** — the claim relayed into the dispatch was false as stated; the spelling that actually demonstrates the weak assertion is mixed-case `Europe/Kiev`, which passed **both** probes at 0. (c) **`tests/probe/` HAS a `README.md`** (12 KB, with the probe table); the board and record both said it had none — it documented probe09's precondition and not probe09b's, and now documents both. (d) **`TM-114`, and it is probably shared:** `BUILD.md` §3 was missing the compiler's default `compile` stage and assigned `tests/conformance/` to `accept` — *accepted in silence*, the shape O-N11 walks through. **The template was shared, so each sibling should check its own §3 at its next claim.** **CI HAS NEVER BEEN GREEN AND CANNOT BE FROM HERE** — 0.0.1 does not push. The workflow pins `0dfddac` (full SHA) and LLVM **20.1.2** exactly, asserts a clean compiler checkout and the ladder's working directory, and its YAML plus all six `run:` blocks validate locally. **Make the cycle-closing push the first run** and record its id, as `nitpick-regex` does. Original dispatch carried **six items, five accrued after 0.0.1 was planned** — the subcycle's own six steps, plus: the two DEF-5 transcripts re-recorded against the measured after-value (`NITPICK-REACH-003`, **four** identities, not six); `expect-` headers for the three `missing_failsafe` cases; the three stale `list.npk` citations in `probe06_generic_vec.npk` (lines **14, 92, 107** — re-derived with an instrument that can see the checkouts); `probe09b`'s undocumented `TZ=Europe/Kiev` precondition, on `probe09_environ_split`'s self-announcing pattern; and **step 4's CI pin, which now has a measured answer** — `0dfddac`, LLVM **20.1.2 exactly** (patch release included), ladder from the tree root. **ALL FOUR STOPS ARE DOWN**, each on our own measurement: O-N4 (orchestrator, re-confirmed by the worker re-running probe 04 whole), O-N9 (TM-110), O-N10 (TM-111), O-N11 (TM-112). No compiler defect found. **0.0.5 and 0.5 unblocked.** **RESIDUE FOR 0.0.1, found by the orchestrator reading the verifier's denominator:** the expect-header sweep covered **36 of 42** tracked `.npk`. Three of the six uncovered are support libraries and correctly have no header. **The other three are `tests/probe/defect/missing_failsafe/case1`, `case2` and `case3` — defect reproductions with NO `expect-` header at all**, and they are precisely the files whose expected behaviour changed today (an `npkc` `NITPICK-REACH-003` refusal replaced an `llc` failure). The worker's own finding is that a defect reproduction's header goes stale the day its defect is fixed and that the sweep is the check; these three sit OUTSIDE that sweep, so the strengthened check cannot see the files it most needed to. Give them headers at 0.0.1 and make the sweep assert its DENOMINATOR (stream 1's denominator finding — no number invented for it here; `nitpick-regex` allocated RX-123 through RX-126 today, verified by `git grep '^### RX-'`, so its next free is RX-127) so an uncovered file is a finding rather than a silence
